...
MAX_WRENCH_FORCE = 25.0
MAX_WRENCH_TORQUE = 20.0
...
class CtrlCfg:
    ...
    gravity_comp_bias_min = [0.0, -1.0, 0.0, -0.5, 0.0, 0.0, 0.0]  # Min bias per joint (N-m)
    gravity_comp_bias_max = [0.0,  0.5, 0.0,  0.5, 0.0, 0.0, 0.0]  # Max bias per joint (N-m)
    gravity_comp_noise_std = [0.0,  0.05, 0.0,  0.05, 0.0, 0.0, 0.0]  # Per-step noise std per joint (N-m)
...
def compute_dof_torque(
    ...
    # dof_torque[:, 1] += torch.tensor([-1.0], device=device)
    # dof_torque[:, 3] += torch.tensor([0.5], device=device)
