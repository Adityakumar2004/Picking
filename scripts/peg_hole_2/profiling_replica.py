...
from scripts.peg_hole_2.env_replica_real import Agent
...
class RunningNormalizer:
    ...
    def normalize(self, x: np.ndarray):
        ...
        print(f"x.shape: {x.shape}, self.mean.shape: {self.mean.shape}, self.var.shape: {self.var.shape}")
        std = np.sqrt(self.var) + 1e-8
        x_norm = (x - self.mean) / std
        return np.clip(x_norm, -self.clip_range, self.clip_range)

class env_wrapper(gym.Wrapper):
    def __init__(self, env, enable_normalization_obs:bool=True):
        super().__init__(env)
        self.env = env
        ...
        if self.enable_normalization_obs:
            self.normalizers = RunningNormalizer(19, clip_range=8.0)
    ...
    def step(self, actions):
        ...
        if isinstance(actions, np.ndarray):
            actions = torch.from_numpy(actions).float().to(self.unwrapped.device)
            actions = torch.clamp(actions, -1.0, 1.0)
        obs, rewards, terminations, truncations, info = super().step(actions)

        if self.enable_normalization_obs:
            obs_policy_np = obs["policy"]
            if isinstance(obs["policy"], torch.Tensor):
                obs_policy_np = obs["policy"].cpu().numpy()
            obs["policy"] = torch.tensor(self.normalizers.normalize(obs_policy_np), device=self.unwrapped.device, dtype=torch.float32)
...
def make_env(video_folder:str | None =None, output_type: str = "numpy"):
    ...
    gym.register(
        id=id_name,
        entry_point="scripts.peg_hole_2.env_replica_real:RobotEnvLogging",
        disable_env_checker=True,
        kwargs={
            "env_cfg_entry_point":"scripts.peg_hole_2.env_replica_real:FactoryTaskPegInsertCfg",
        },
    )
    ...
    env = env_wrapper(env, enable_normalization_obs=True)
    
    return env

def main():
    env = make_env(video_folder=None, output_type="numpy")
    env.unwrapped.enable_replica_changes(True)

    ### ------------- setting up the agent and loading the checkpoint -------------

    agent = Agent(eval=True)
    agent.to(env.unwrapped.device)
    checkpoint_path = "scripts/peg_hole_2/logs/ppo_factory/checkpoints/diff_ik_low_T_high_bounds.pt"
    ...
    if hasattr(env, 'normalizers'):
        ...
        state = checkpoint['normalizer_state']['policy']
        env.normalizers.mean = state['mean']
        env.normalizers.var = state['var']
        env.normalizers.count = state['count']
        env.normalizers.clip_range = state['clip_range']
        print("shape of normalizer mean:", env.normalizers.mean.shape)
        print("shape of normalizer var:", env.normalizers.var.shape)
    
    else:
        print("No normalizers found in the environment wrapper.")
        sys.exit(1)
        ...
        with torch.no_grad():
            ...
            if execute_trajectory:
                ...
                if recording_step == 0:
                    ...
                    torch.zeros(agent.num_layers, 1, agent.hidden_size).to(env.unwrapped.device),
                    torch.zeros(agent.num_layers, 1, agent.hidden_size).to(env.unwrapped.device),
                    )
                    dones = torch.zeros(1, device=env.unwrapped.device)

                # print(f"recording step: {recording_step}, obs: \n {obs} ")

                pose_action, _, _, next_lstm_state_actor, a_mu, a_std= agent.get_action(obs, next_lstm_state_actor, dones)
                pose_action = pose_action.cpu().numpy().flatten()
                pose_action = np.clip(pose_action, -1.0, 1.0)
                print(f"recording step: {recording_step} action: {pose_action}")
        
                if recording_step >= 200:
                    recording_step = 0
                    execute_trajectory = False
            ...
            pose_action[:3] = pose_action[:3] * 0.02 
