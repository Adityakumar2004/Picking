...
class env_wrapper(gym.Wrapper):
    ...
    def __init__(self, env, video_folder:str | None =None, output_type: str = "numpy", enable_normalization_obs = True, enable_normalization_rewards = False):
        super().__init__(env)
        self.env = env
        self.total_obs_space = {"policy": env.unwrapped.observation_space, "critic": env.unwrapped.state_space}
    ...
    def step(self, actions):
        ...
        if self.enable_normalization_obs:
            ...
            with torch.no_grad():
                obs["policy"] = torch.tensor(self.normalizers["policy"].normalize(obs["policy"].cpu().numpy()), device=self.env.unwrapped.device)
                obs["critic"] = torch.tensor(self.normalizers["critic"].normalize(obs["critic"].cpu().numpy()), device=self.env.unwrapped.device)
        
        if self.enable_normalization_rewards:
            if self.training:
                self.normalizers["rewards"].update(rewards.cpu().numpy())
                rewards = torch.tensor(self.normalizers["rewards"].normalize(rewards.cpu().numpy()), device=self.env.unwrapped.device)
    ...
    def set_camera_pose_fixed_asset(self, camera_id, env_id):
        ...
        camera_target = fixed_asset_default_state[:3] + torch.tensor([0.0, 0.0, 0.005], device=self.unwrapped.device) + self.unwrapped.scene.env_origins[env_id]
    ...
    def calculate_rewards(self, output_type, rewards_task2):
        ...
        if output_type == "numpy":
            ...
            if isinstance(reward, np.ndarray):
                reward = torch.tensor(reward, device=self.env.unwrapped.device)
