...
MAX_WRENCH_FORCE = 20.0
MAX_WRENCH_TORQUE = 15.0

@configclass
class CtrlCfg:
    ema_factor = 0.2

    pos_action_bounds = [0.06, 0.06, 0.06]
    ...
    default_task_prop_gains = [200, 200, 200, 60, 60, 60]#[100, 100, 100, 30, 30, 30]
    ...
    # Bias is sampled uniformly from [min, max] per episode, noise_std adds per-step Gaussian noise
    # Set std=0 for constant bias throughout episode, set min=max=0 to disable for a joint
    gravity_comp_noise_enabled = False
    gravity_comp_bias_min = [0.0, -5.0, 0.0, -1.5, 0.0, 0.0, 0.0]  # Min bias per joint (N-m)
    gravity_comp_bias_max = [0.0,  1.5, 0.0,  1.5, 0.0, 0.0, 0.0]  # Max bias per joint (N-m)
    gravity_comp_noise_std = [0.0,  0.005, 0.0,  0.05, 0.0, 0.0, 0.0]  # Per-step noise std per joint (N-m)

@configclass
class FactoryEnvCfg(DirectRLEnvCfg):
    default_gripper_joint_pos = np.array([1, 1.0, 1.0, 1, -0.8, -0.8])*0.07
    ## rounding the numbers and converting to list to avoid precision issues while comparing with the config
    default_gripper_joint_pos = np.round(default_gripper_joint_pos, decimals=4).tolist()
    decimation = 8
    action_space = 6
    ...
    robot = ArticulationCfg(
        ...
        init_state=ArticulationCfg.InitialStateCfg(
            ...
            joint_pos={
                ...
                "gen3_joint_6": -1.5667,
                "gen3_joint_7": -1.5310,
                "finger_joint": default_gripper_joint_pos[0], #0.4, # 0, 0.8
                "left_inner_knuckle_joint": default_gripper_joint_pos[1], #0.1, # 0, 0.8757
                "right_inner_knuckle_joint": default_gripper_joint_pos[2], #0.1, # 0, 0.8757
                "right_outer_knuckle_joint": default_gripper_joint_pos[3], #0.1, # 0, 0.81
                "left_inner_finger_joint": default_gripper_joint_pos[4], #-0.08, #-0.8757, 0
                "right_inner_finger_joint": default_gripper_joint_pos[5], #-0.08, #-0.8757, 0
            },
        ),
        actuators={
...
class FactoryEnv(DirectRLEnv):
    ...
    def __init__(self, cfg: FactoryTaskPegInsertCfg, render_mode: str | None = None, **kwargs):
        ...
        self.gripper_joint_ids = self._robot.find_joints(self.gripper_joint_names)[0]
        self.gripper_open_val = torch.tensor([0.3], device=self.device)
        self.gripper_close_val = torch.tensor([0.65], device=self.device)
    ...
    def _init_tensors(self):
        ...
        self.ctrl_target_joint_vel = torch.zeros((self.num_envs, self._robot.num_joints), device=self.device)
        ...
        # Gravity compensation noise bias (sampled per episode, per joint)
        # Shape: (num_envs, 7) for 7 arm joints
        self.gravity_comp_bias = torch.zeros((self.num_envs, 7), device=self.device)
        self.apply_gravity_comp_noise = False  # Only apply after reset completes
    ...
    def _compute_intermediate_values(self, dt):
        ...
        ## TODO check the index if there is any need of -1
        self.fingertip_midpoint_jacobian = jacobians[:, self.fingertip_body_idx, 0:6, 0:7]
    ...
    def _pre_physics_step(self, action):
        ...
        self.ctrl_target_fingertip_midpoint_pos = self.ctrl_target_fingertip_midpoint_pos + pos_actions

        # Clamp target position within env bounds: fixed_pos_action_frame ± pos_action_bounds
        pos_action_bounds = torch.tensor(self.cfg.ctrl.pos_action_bounds, device=self.device)
        lower_bound = self.fixed_pos_action_frame - pos_action_bounds
        upper_bound = self.fixed_pos_action_frame + pos_action_bounds
        self.ctrl_target_fingertip_midpoint_pos = torch.clamp(
            self.ctrl_target_fingertip_midpoint_pos, min=lower_bound, max=upper_bound
        )
    ...
    def close_gripper_in_place(self):
        ...
        ctrl_target_gripper_dof_pos = self.gripper_close_val.repeat((self.num_envs, 1)) ## dim = (num_envs, 1)
        ctrl_target_gripper_dof_pos = ctrl_target_gripper_dof_pos * self.gripper_multiplier ## num_envs,1 * 1,6 -> num_envs,6
    ...
    def generate_ctrl_signals(self):
        ...
        self.joint_torque, self.applied_wrench = compute_dof_torque(
            cfg=self.cfg,
            ...
            device=self.device,
            gravity_comp_bias=self.gravity_comp_bias if (self.cfg.ctrl.gravity_comp_noise_enabled and self.apply_gravity_comp_noise) else None,
        )
    ...
    def _reset_idx(self, env_ids):
        ...
        # Disable gravity comp noise during reset (IK solving, grasping, etc.)
        self.apply_gravity_comp_noise = False

        # Sample new gravity compensation bias for this episode (per joint)
        if self.cfg.ctrl.gravity_comp_noise_enabled:
            n_envs = len(env_ids)
            bias_min = torch.tensor(self.cfg.ctrl.gravity_comp_bias_min, device=self.device)
            bias_max = torch.tensor(self.cfg.ctrl.gravity_comp_bias_max, device=self.device)
            # Uniform sample from [min, max] for each joint
            rand_samples = torch.rand((n_envs, 7), device=self.device)
            self.gravity_comp_bias[env_ids] = bias_min + rand_samples * (bias_max - bias_min)

        self._set_assets_to_default_pose(env_ids)
        
        # self._robot.set_joint_velocity_target(torch.zeros_like(self.joint_vel), env_ids=env_ids)
        zero_vel = torch.zeros((len(env_ids), self._robot.num_joints), device=self.device)
        self._robot.set_joint_velocity_target(zero_vel[:, :7], joint_ids=range(7), env_ids=env_ids)
        ...
        ## putting current pose as the target pose
        ...
        # Enable gravity comp noise now that reset is complete
        self.apply_gravity_comp_noise = True
    ...
    def set_pos_inverse_kinematics(self, env_ids):
        ...
        # Debug: Check current velocity targets
        # current_vel_targets = self._robot.data.joint_vel[env_ids, :7]
        # if torch.any(torch.abs(current_vel_targets) > 0.01):
        #     print(f"WARNING: Non-zero velocity targets detected before IK: {current_vel_targets[0]}")
        ...
        while ik_time < 0.25:
            ...
            # Solve DLS problem.
            delta_dof_pos = _get_delta_dof_pos(
            ...
            self.ctrl_target_joint_vel[env_ids, :] = self.joint_vel[env_ids, :]
            # Update dof state.
    ...
    def get_handheld_asset_relative_pose(self):
        ...
        if self.cfg_task.name == "peg_insert":
            ...
            held_asset_relative_pos[:, 2] -= self.cfg_task.robot_cfg.franka_fingerpad_length - 0.005
        else:
            raise NotImplementedError("Task not implemented")
    ...
    def _set_franka_to_default_pose(self, joints, env_ids):
        ...
        self.ctrl_target_joint_vel[env_ids, :] = joint_vel
        self._robot.set_joint_position_target(self.ctrl_target_joint_pos[env_ids, 7:], self.gripper_joint_ids, env_ids=env_ids)
        # self._robot.set_joint_velocity_target(self.ctrl_target_joint_vel[env_ids, 7:], self.gripper_joint_ids, env_ids=env_ids)
        # self._robot.set_joint_velocity_target(self.ctrl_target_joint_vel[env_ids, :7], joint_ids=range(7), env_ids=env_ids)
    ...
    def randomize_near_goal_state(self, env_ids):
        ...
        while True:
            ...
            pos_error_mask = pos_error_norm > 1e-3
            angle_error_mask = angle_error_norm > 5e-3  # Relaxed from 1e-3 (0.057 deg) to 5e-3 (0.29 deg)
            any_error = torch.logical_or(pos_error_mask, angle_error_mask)

            # Get indices of bad envs (handle edge case when only 1 env fails)
            error_indices = any_error.nonzero(as_tuple=False).squeeze(-1)
            if error_indices.dim() == 0:
                error_indices = error_indices.unsqueeze(0)
            bad_envs = bad_envs[error_indices]

            # Handle case where bad_envs becomes 0-dimensional
            if bad_envs.dim() == 0:
                bad_envs = bad_envs.unsqueeze(0)

            # Check IK succeeded for all envs, otherwise try again for those envs
            print("ik_attempt ", ik_attempt, " bad_envs: ", bad_envs.shape[0])
            if bad_envs.shape[0] == 0:
                break

            # Debug: print error magnitudes when IK is struggling
            if ik_attempt > 10:
                print(f"  pos_error_norm: {pos_error_norm[error_indices]}")
                print(f"  angle_error_norm: {angle_error_norm[error_indices]}")
                print(f"  target_pos: {self.ctrl_target_fingertip_midpoint_pos[bad_envs[0]]}")
                print(f"  current_pos: {self.fingertip_midpoint_pos[bad_envs[0]]}")

            # Limit max IK attempts to prevent infinite loop
            if ik_attempt >= 50:
                print(f"WARNING: IK failed to converge after {ik_attempt} attempts for {bad_envs.shape[0]} envs. Proceeding anyway.")
                break
    ...
    def noising_relative_pos_wrt_hole_centre(self, n_bad, radius, theta, height, radius_noise_bound, theta_noise_bound, height_noise_bound):
        ...
        ## [-0.7, 0.3] * height_noise_bound
        height_noise = (np.random.rand(n_bad) - 0.7)*height_noise_bound
    ...
    def custom_hand_pos_randomization(self, bad_envs, world_coords=True, fixed_asset_pos=None):
        ...
        height_wrt_hole = -0.012 # 0 
        height_noise_bounds = 0.003#0.008

        outer_radius = 5*asset_info["fixed_asset_diameter"]/2 + 0.03#0.0045
        # outer_radius = 5*asset_info["fixed_asset_diameter"]/2 + 0.009
        radius_noise_bound = 0.004#0.004
...
class LoggingWrapper(FactoryEnv):
    ...
    def _setup_scene(self):
        ...
        # Instantiate the camera sensors
        self.cameras = [Camera(self.camera1_cfg), Camera(self.camera2_cfg)]
...
def compute_dof_torque(
    ...
    dof_torque[:, 0:3] = torch.clamp(dof_torque[:, 0:3], min=-MAX_WRENCH_FORCE, max=MAX_WRENCH_FORCE)
    dof_torque[:, 3:7] = torch.clamp(dof_torque[:, 3:7], min=-MAX_WRENCH_TORQUE, max=MAX_WRENCH_TORQUE)
    ...
    # Apply gravity compensation noise to all 7 arm joints
    # gravity_comp_bias: (num_envs, 7) - per-episode bias sampled from [min, max]
    # noise_std: per-step Gaussian noise added on top of bias
    if gravity_comp_bias is not None:
        noise_std = torch.tensor(cfg.ctrl.gravity_comp_noise_std, device=device)
        # Gaussian noise: mean = bias (sampled per episode), std from config
        joint_noise = gravity_comp_bias + noise_std * torch.randn(num_envs, 7, device=device)
        dof_torque[:, 0:7] += joint_noise

    dof_torque = torch.clamp(dof_torque, min=-490, max=490)
